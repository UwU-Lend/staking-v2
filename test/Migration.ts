...
describe("Migration", () => {
  ...
  describe("BalanceOf", () => {
    it("Should be the right balance of account", async () => {
      ...
      await migration.setBalances(account1.address, balances1);
      await migration.setBalances(account2.address, balances2);
      expect(await migration.balanceOf(account1.address)).to.be.equals(BigNumber.from(400));
      expect(await migration.balanceOf(account2.address)).to.be.equals(BigNumber.from(500));
    });
  });
  describe("TotalSupply", () => {
    it("Should be the right total supply", async () => {
      ...
      await migration.setBalances(account1.address, balances1);
      await migration.setBalances(account2.address, balances2);
      expect(await migration.totalSupply()).to.be.equals(BigNumber.from(900));
    });
  });
});